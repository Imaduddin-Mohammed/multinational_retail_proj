...
if __name__ == "__main__":
    ...
    connector.upload_to_db(cleaned_products_df, "dim_products", engine2)
    ...
    connector.upload_to_db(cleaned_orders_df, 'dim_orders', engine2)
    print("Successfully uploaded orders data as 'dim_orders' in sales database")

    # EXTRACTING JSON FILE STORED ON AWS S3 BUCKET:
    json_file_path= config('JSON_FILE_PATH')
    date_times = extractor.retrieve_json_data(json_file_path)

    # CLEANING EXTRACTED DATE DETAILS:
    cleaned_date_time_details = cleaner.clean_date_times(date_times)
    print(cleaned_date_time_details.info())

    # UPLOADING DATE TIME DETAILS TO SALES DATABASE:
    connector.upload_to_db(cleaned_date_time_details, 'dim_date_times', engine2)
    print("Successfully uploaded date time details to sales database")
